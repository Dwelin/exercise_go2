...
var once sync.Once
var internalVarifyCode *VerifyCode

// NewVerifyCode 单例模式获取
func NewVerifyCode() *VerifyCode {
	once.Do(func() {
		internalVarifyCode = &VerifyCode{
			Store: &RedisStore{
...
func (vc *VerifyCode) CheckAnswer(key string, answer string) bool {
	...
	if !app.IsProduction() && (strings.HasSuffix(key, config.GetString("verifycode.debug_email_suffix")) || strings.HasPrefix(key, config.GetString("verifycode.debug_phone_suffix"))) {
		return true
	}
